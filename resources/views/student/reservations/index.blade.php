<x-app-layout title="Mis reservas - QuickWash Campus">
    ...
    <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div class="overflow-x-auto"><table class="min-w-full text-left text-sm">
            <thead class="bg-slate-100 text-xs uppercase text-slate-600"><tr><th class="p-4">Lavadora</th><th class="p-4">Fecha</th><th class="p-4">Hora</th><th class="p-4">Prendas</th><th class="p-4">Estado</th><th class="p-4">Acciones</th></tr></thead>
            <tbody class="divide-y divide-slate-100">
                @forelse($reservations as $reservation)
                    <tr><td class="p-4 font-medium">{{ $reservation->washer->code }}</td><td class="p-4">{{ $reservation->reservation_date->format('d/m/Y') }}</td><td class="p-4">{{ $reservation->reservation_time }}</td><td class="p-4">{{ $reservation->garments_count }}</td><td class="p-4"><span class="badge {{ $reservation->status }}">{{ str_replace('_', ' ', $reservation->status) }}</span></td><td class="p-4">@if($reservation->status === 'pending' && ! $reservation->isPastSlot())<form method="POST" action="{{ route('student.reservations.cancel', $reservation) }}">@csrf @method('PATCH')<button class="btn-danger">Cancelar</button></form>@else<span class="text-slate-400">Sin acción</span>@endif</td></tr>
                @empty
