<x-app-layout title="Reservas - QuikWash">
    ...
    <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><div class="overflow-x-auto"><table class="min-w-full text-left text-sm">
        <thead class="bg-slate-100 text-xs uppercase text-slate-600"><tr><th class="p-4">Estudiante</th><th class="p-4">Lavadora</th><th class="p-4">Fecha</th><th class="p-4">Kg</th><th class="p-4">Notas</th><th class="p-4">Estado</th><th class="p-4">Cambiar</th></tr></thead>
        <tbody class="divide-y divide-slate-100">
            @forelse($reservations as $reservation)
                <tr><td class="p-4">{{ $reservation->user->fullName() }}</td><td class="p-4">{{ $reservation->washer->code }}</td><td class="p-4">{{ $reservation->reservation_date->format('d/m/Y') }} {{ $reservation->reservation_time }}</td><td class="p-4">{{ $reservation->weight_kg }}</td><td class="p-4 text-slate-600">{{ $reservation->notes ?: '-' }}</td><td class="p-4"><span class="badge {{ $reservation->status }}">{{ __('labels.reservation_status.'.$reservation->status) }}</span></td><td class="p-4">
